...
class NetworkModuleHada(Lora.network.NetworkModule):
    ...
    def calc_updown(self, orig_weight):
        ...
        if self.t1 is not None:
            ...
            updown1 = Lora.lyco_helpers.make_weight_cp(t1, w1a, w1b)
            output_shape += t1.shape[2:]
        else:
            if len(w1b.shape) == 4:
                output_shape += w1b.shape[2:]
            updown1 = Lora.lyco_helpers.rebuild_conventional(w1a, w1b, output_shape)

        if self.t2 is not None:
            t2 = self.t2.to(orig_weight.device, dtype=orig_weight.dtype)
            updown2 = Lora.lyco_helpers.make_weight_cp(t2, w2a, w2b)
        else:
            updown2 = Lora.lyco_helpers.rebuild_conventional(w2a, w2b, output_shape)
